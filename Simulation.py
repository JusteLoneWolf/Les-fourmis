...
def Next(choix):       #Cette fonction est lancée depuis Menu_Accueil, elle permet de savoir si l'on doit accéder à la simulation  
...
def placementNourritureDebut(taille, pas, PosNourriture):   #Cette fonction permet de placer la nourriture au lancement de la simulation
    ...
    while NourritureActuelle<nourritureDebut:  #Tant que notre variable NourritureActuelle n'est pas égale à la nourriture qu'il devrait y avoir au début    
        if(PosNourriture=="Aleatoire"):     #On regarde si elle doit être positionnée par défaut ou aléatoirement
            ...
            XNourriture+=0.5    #Les variables s'incrémentent, afin de ne pas superposer les nourriture positionnées par défaut
            YNourriture +=0.5
...
def Sim_Fourmiliere(Taille,NBObstacle, NBMaxFourmis,NBTourSansManger, PosNourriture ):  #Il s'agit du programme principal de la fourmilière
    ...
    while Simulation_Running == True:
        ...
        fourmiliere_OK = pygame.transform.scale(pygame.image.load("fourmiliere.jpg"), (int(Pas), int(Pas)))
        if(int(Taille)%2==0):
        ...
        for laFourmi in listeFourmi:        #Pour chaque fourmi
            if(laFourmi.Faim > 0 and laFourmi.TourMaxRestant > 0 and laFourmi.vie == True):     #On vérifie qu'elle est toujours apte à vivre
                clock.tick(5)       #On explique qu'il y aura 5 images par secondes 
        ...
        for event in pygame.event.get():    #Pour chacun des évènement récupérés par Pygame
            if(event.type==pygame.QUIT):              #Si on clique sur la croix
                Simulation_Running = False      #Alors la simulation s'arrête
                screen.fill(couleur_noir)
                
        if(nbFourmi==0):    #Si le nombre de fourmi est égal à 0
            Simulation_Running=False    #Alors on arrête la simulation
            
        #HEADER
        back_header = pygame.Rect((0,0), (950, 90))
        surf_back_header = pygame.Surface(back_header.size)

       
        imagefour = pygame.image.load("fourmi.png")
        rectfour = imagefour.get_rect()
        rectfour.x = 500
        rectfour.y = 20

        imagenourr = pygame.image.load("Nourriture.jpg") 
        #imagenourr = pygame.transform.scale(pygame.image.load("Nourriture.jpg"), (800,800)) 
